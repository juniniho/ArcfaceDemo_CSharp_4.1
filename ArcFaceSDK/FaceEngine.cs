...
namespace ArcFaceSDK
{
    ...
    {
        public const int maxFaceCount = 50;
        ...
        /// <summary>
        /// 设置人脸属性阈值，取值范围[0-1]， 默认阈值均为:0.5， 用户可以根据实际需求，设置遮挡范围
        /// </summary>
        /// <param name="attributeThreshold">人脸属性阈值</param>
        /// <returns>返回0表示正常；其他值请在官网-帮助中心查询</returns>
        public int ASFSetFaceShelterParam(ASF_FaceAttributeThreshold attributeThreshold)
        {
            return ASFFunctions.ASFSetFaceAttributeParam(pEngine, attributeThreshold);
        }
        ...
        public int ASFDetectFaces(Image image, out MultiFaceInfo multiFaceInfo, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, ASF_DetectModel detectModel = ASF_DetectModel.ASF_DETECT_MODEL_RGB, bool isEx = false)
        {
            ...
            if (multiFaceInfo.faceNum > 0 && multiFaceInfo.faceNum < maxFaceCount)
            {
                ...
                multiFaceInfo.foreheadRect = new MRECT[multiFaceInfo.faceNum];
                for (int i = 0; i < multiFaceInfo.faceNum; i++)
                {
                    multiFaceInfo.foreheadRect[i] = MemoryUtil.PtrToStructure<MRECT>(multiFaceInfoStruct.foreheadRect + MemoryUtil.SizeOf<MRECT>() * i);
                }

                multiFaceInfo.faceAttributeInfo = new FaceAttributeInfo();
                ASF_FaceAttributeInfo aSF_FaceAttribute = multiFaceInfoStruct.faceAttributeInfo;
                if (aSF_FaceAttribute.wearGlasses != IntPtr.Zero)
                {
                    multiFaceInfo.faceAttributeInfo.wearGlasses = new int[multiFaceInfo.faceNum];
                    Marshal.Copy(aSF_FaceAttribute.wearGlasses, multiFaceInfo.faceAttributeInfo.wearGlasses, 0, multiFaceInfo.faceNum);
                }
                if(aSF_FaceAttribute.leftEyeOpen != IntPtr.Zero)
                {
                    multiFaceInfo.faceAttributeInfo.leftEyeOpen = new int[multiFaceInfo.faceNum];
                    Marshal.Copy(aSF_FaceAttribute.leftEyeOpen, multiFaceInfo.faceAttributeInfo.leftEyeOpen, 0, multiFaceInfo.faceNum);
                }
                if(aSF_FaceAttribute.rightEyeOpen != IntPtr.Zero)
                {
                    multiFaceInfo.faceAttributeInfo.rightEyeOpen = new int[multiFaceInfo.faceNum];
                    Marshal.Copy(aSF_FaceAttribute.rightEyeOpen, multiFaceInfo.faceAttributeInfo.rightEyeOpen, 0, multiFaceInfo.faceNum);
                }
                if(aSF_FaceAttribute.mouthClose != IntPtr.Zero)
                {
                    multiFaceInfo.faceAttributeInfo.mouthClose = new int[multiFaceInfo.faceNum];
                    Marshal.Copy(aSF_FaceAttribute.mouthClose, multiFaceInfo.faceAttributeInfo.mouthClose, 0, multiFaceInfo.faceNum);
                }

                multiFaceInfo.face3DAngleInfo = new Face3DAngle();
                ASF_Face3DAngle aSF_Face3DAngle = multiFaceInfoStruct.face3DAngleInfo;

                if(aSF_Face3DAngle.roll != IntPtr.Zero)
                {
                    multiFaceInfo.face3DAngleInfo.roll = new float[multiFaceInfo.faceNum];
                    Marshal.Copy(aSF_Face3DAngle.roll, multiFaceInfo.face3DAngleInfo.roll, 0, multiFaceInfo.faceNum);
                }
                if(aSF_Face3DAngle.yaw != IntPtr.Zero)
                {
                    multiFaceInfo.face3DAngleInfo.yaw = new float[multiFaceInfo.faceNum];
                    Marshal.Copy(aSF_Face3DAngle.yaw, multiFaceInfo.face3DAngleInfo.yaw, 0, multiFaceInfo.faceNum);
                }
                if (aSF_Face3DAngle.pitch != IntPtr.Zero)
                {
                    multiFaceInfo.face3DAngleInfo.pitch = new float[multiFaceInfo.faceNum];
                    Marshal.Copy(aSF_Face3DAngle.pitch, multiFaceInfo.face3DAngleInfo.pitch, 0, multiFaceInfo.faceNum);
                }
        ...
        /// <summary>
        /// 人脸检测/人脸追踪
        /// 图像数据以结构体形式传入，对采用更高字节对齐方式的图像兼容性更好，图像宽度需要四字节对齐（宽度为4的倍数）
        /// </summary>
        /// <param name="image">图片</param>
        /// <param name="multiFaceInfo">多人脸对象</param>
        /// <param name="imageFormat">图片格式</param>
        ...
        public int ASFUpdateFaceData(Image image, ref MultiFaceInfo multiFaceInfo, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, bool isEx = false)
        {
            ...
            if (multiFaceInfo.faceNum > 0 && multiFaceInfo.faceNum <= maxFaceCount)
            {
                ...
                if (multiFaceInfo.faceIsWithinBoundary != null)
                {
                ...
                if (multiFaceInfo.foreheadRect != null)
                {
                    multiFaceInfoStruct.foreheadRect = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<MRECT>());
                    byte[] foreheadRectBytes = new byte[MemoryUtil.SizeOf<MRECT>() * multiFaceInfo.faceNum];
                    for (int i = 0; i < multiFaceInfo.faceNum; i++)
                    {
                        byte[] tempBytes = MemoryUtil.StructToBytes(multiFaceInfo.foreheadRect[i]);
                        tempBytes.CopyTo(foreheadRectBytes, MemoryUtil.SizeOf<MRECT>() * i);
                    }
                    Marshal.Copy(foreheadRectBytes, 0, multiFaceInfoStruct.foreheadRect, foreheadRectBytes.Length);
                }

                if (multiFaceInfo.faceAttributeInfo != null)
                {
                    ASF_FaceAttributeInfo asfFaceAttributeInfo = new ASF_FaceAttributeInfo();
                    if (multiFaceInfo.faceAttributeInfo.wearGlasses != null)
                    {
                        asfFaceAttributeInfo.wearGlasses = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<int>());
                        Marshal.Copy(multiFaceInfo.faceAttributeInfo.wearGlasses, 0, asfFaceAttributeInfo.wearGlasses, multiFaceInfo.faceNum);
                        faceDataInfoIntPtrList.Add(asfFaceAttributeInfo.wearGlasses);
                    }
                    if (multiFaceInfo.faceAttributeInfo.leftEyeOpen != null)
                    {
                        asfFaceAttributeInfo.leftEyeOpen = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<int>());
                        Marshal.Copy(multiFaceInfo.faceAttributeInfo.leftEyeOpen, 0, asfFaceAttributeInfo.leftEyeOpen, multiFaceInfo.faceNum);
                        faceDataInfoIntPtrList.Add(asfFaceAttributeInfo.leftEyeOpen);
                    }
                    if (multiFaceInfo.faceAttributeInfo.rightEyeOpen != null)
                    {
                        asfFaceAttributeInfo.rightEyeOpen = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<int>());
                        Marshal.Copy(multiFaceInfo.faceAttributeInfo.rightEyeOpen, 0, asfFaceAttributeInfo.rightEyeOpen, multiFaceInfo.faceNum);
                        faceDataInfoIntPtrList.Add(asfFaceAttributeInfo.rightEyeOpen);
                    }
                    if (multiFaceInfo.faceAttributeInfo.mouthClose != null)
                    {
                        asfFaceAttributeInfo.mouthClose = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<int>());
                        Marshal.Copy(multiFaceInfo.faceAttributeInfo.mouthClose, 0, asfFaceAttributeInfo.mouthClose, multiFaceInfo.faceNum);
                        faceDataInfoIntPtrList.Add(asfFaceAttributeInfo.mouthClose);
                    }
                    multiFaceInfoStruct.faceAttributeInfo = asfFaceAttributeInfo;
                }

                if (multiFaceInfo.face3DAngleInfo != null)
                {
                    ASF_Face3DAngle asfFace3DAngle = new ASF_Face3DAngle();
                    if (multiFaceInfo.face3DAngleInfo.pitch != null)
                    {
                        asfFace3DAngle.pitch = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<float>());
                        Marshal.Copy(multiFaceInfo.face3DAngleInfo.pitch, 0, asfFace3DAngle.pitch, multiFaceInfo.faceNum);
                        faceDataInfoIntPtrList.Add(asfFace3DAngle.pitch);
                    }
                    if (multiFaceInfo.face3DAngleInfo.roll != null)
                    {
                        asfFace3DAngle.roll = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<float>());
                        Marshal.Copy(multiFaceInfo.face3DAngleInfo.roll, 0, asfFace3DAngle.roll, multiFaceInfo.faceNum);
                        faceDataInfoIntPtrList.Add(asfFace3DAngle.roll);
                    }
                    if (multiFaceInfo.face3DAngleInfo.yaw != null)
                    {
                        asfFace3DAngle.yaw = MemoryUtil.Malloc(multiFaceInfo.faceNum * MemoryUtil.SizeOf<float>());
                        Marshal.Copy(multiFaceInfo.face3DAngleInfo.yaw, 0, asfFace3DAngle.yaw, multiFaceInfo.faceNum);
                        faceDataInfoIntPtrList.Add(asfFace3DAngle.yaw);
                    }
                    multiFaceInfoStruct.face3DAngleInfo = asfFace3DAngle;
                }
            ...
            if(retCode != 0)
            {
                MemoryUtil.FreeArray(pMultiFaceInfo, imageInfo.imgData, multiFaceInfoStruct.faceDataInfoList,multiFaceInfoStruct.faceID, multiFaceInfoStruct.faceOrient, multiFaceInfoStruct.faceRect, multiFaceInfoStruct.faceIsWithinBoundary, multiFaceInfoStruct.foreheadRect);
            ...
            if (multiFaceInfo.faceNum > 0)
            {
                ...
                for (int i = 0; i < multiFaceInfo.faceNum; i++)
                {
                    multiFaceInfo.foreheadRect[i] = MemoryUtil.PtrToStructure<MRECT>(multiFaceInfoStructNew.foreheadRect + MemoryUtil.SizeOf<MRECT>() * i);
                }

                ASF_FaceAttributeInfo aSF_FaceAttribute = multiFaceInfoStructNew.faceAttributeInfo;
                if(aSF_FaceAttribute.wearGlasses != IntPtr.Zero)
                {
                    multiFaceInfo.faceAttributeInfo.wearGlasses = new int[multiFaceInfo.faceNum];
                    Marshal.Copy(aSF_FaceAttribute.wearGlasses, multiFaceInfo.faceAttributeInfo.wearGlasses, 0, multiFaceInfo.faceNum);
               ...
               if(aSF_FaceAttribute.rightEyeOpen != IntPtr.Zero)
                ...
                ASF_Face3DAngle aSF_Face3DAngle = multiFaceInfoStructNew.face3DAngleInfo;
                if(aSF_Face3DAngle.pitch != IntPtr.Zero)
                {
                    multiFaceInfo.face3DAngleInfo.pitch = new float[multiFaceInfo.faceNum];
                    Marshal.Copy(aSF_Face3DAngle.pitch, multiFaceInfo.face3DAngleInfo.pitch, 0, multiFaceInfo.faceNum);
        ...
        public int ASFProcess(Image image, MultiFaceInfo multiFaceInfo, int combinedMask, ASF_ImagePixelFormat imageFormat = ASF_ImagePixelFormat.ASVL_PAF_RGB24_B8G8R8, bool isEx = false)
        {
            ...
            //释放内存
            MemoryUtil.FreeArray(imageInfo.imgData, multiFaceInfoStruct.faceDataInfoList, multiFaceInfoStruct.faceID, multiFaceInfoStruct.faceOrient, multiFaceInfoStruct.faceRect, multiFaceInfoStruct.faceIsWithinBoundary, multiFaceInfoStruct.foreheadRect, pMultiFaceInfo);
            MemoryUtil.FreeList(faceDataInfoIntPtrList);
            return retCode;
        }
        ...
        public int ASFSetLivenessParam(float rgbThreshold = 0.5f, float irThreshole = 0.7f)
        {
            ...
            livebessThreshold.thresholdmodel_FQ = 0.65f;
