...
namespace ArcSoftFace
{
    public partial class FaceForm : Form
    {
        ...
        private void ChooseImg(object sender, EventArgs e)
        {
            try
            {
                ...
                if (openFileDialog.ShowDialog().Equals(DialogResult.OK))
                {
                    ...
                    Face3DAngle face3DAngleInfo = multiFaceInfo.face3DAngleInfo;
                    ...
                    for (int i = 0; i < multiFaceInfo.faceNum; i++)
                    {
                        ...
                        ageTemp[i] = age;
                        genderTemp[i] = gender;
                        AppendText(string.Format("{0} - 第{1}人脸坐标:[left:{2},top:{3},right:{4},bottom:{5},\r\norient:{6},roll:{7},pitch:{8},yaw:{9},wearGlasses:{10},\r\nleftEyeOpen:{11},rightEyeOpen:{12},mouthClose:{13}],Age:{14},Gender:{15},Mask:{16}\r\n--------------------------------------------------------",
                            DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss"), i, rect.left, rect.top, rect.right, rect.bottom, orient, roll, pitch, yaw, multiFaceInfo.faceAttributeInfo.wearGlasses[i].ToString("f2"), multiFaceInfo.faceAttributeInfo.leftEyeOpen[i], multiFaceInfo.faceAttributeInfo.rightEyeOpen[i],
                            multiFaceInfo.faceAttributeInfo.mouthClose[i], age, (gender >= 0 ? gender.ToString() : ""), (isMask ? "mask" : "no mask")));
                    }
                    AppendText(string.Format("--------------检测结束，时间:{0}--------------", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss:ms")));
                    AppendText("");
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        rgbDeviceVideo = new VideoCaptureDevice(filterInfoCollection[rgbCameraIndex <= maxCameraCount ? rgbCameraIndex : 0].MonikerString);
                        //选择分辨率，需要可以打开注释
                        //GetDeviceResolution(rgbDeviceVideo);
                        rgbVideoSource.VideoSource = rgbDeviceVideo;
                        rgbVideoSource.Start();
        ...
        {
            var videoCapabilities = videoCaptureDevice.VideoCapabilities;//设备的摄像头分辨率数组
            foreach (VideoCapabilities capabilty in videoCapabilities)
            {
                //把这个设备的所有分辨率添加到列表
                string res = $"{capabilty.FrameSize.Width} x {capabilty.FrameSize.Height}";
                Console.WriteLine(res);
            }
            videoCaptureDevice.VideoResolution = videoCapabilities[2];//1920*1080
        ...
        private void videoSource_Paint(object sender, PaintEventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    //bitmapClone = (Bitmap)ImageUtil.CutImage(bitmapClone, 0, 0, 1500, 1080);
                    //检测人脸，得到Rect框
                    MultiFaceInfo multiFaceInfo;
                    int retCode = videoEngine.ASFDetectFacesEx(bitmapClone, out multiFaceInfo);
                    ...
                    for (int faceIndex = 0; faceIndex < multiFaceInfo.faceNum; faceIndex++)
                    {
                        ...
                        lock (rgbVideoImageLocker)
                        {
                            if (multiFaceInfo.foreheadRect != null && multiFaceInfo.foreheadRect.Length > 0)
                            {
                                
                                for (int i = 0; i < multiFaceInfo.foreheadRect.Length; i++)
                                {
                                    PointF[] points = new PointF[4];
                                    MRECT mRECT = multiFaceInfo.foreheadRect[i];
                                    points[0].X = mRECT.left;
                                    points[0].Y = mRECT.top;

                                    points[1].X = mRECT.right;
                                    points[1].Y = mRECT.top;

                                    points[2].X = mRECT.right;
                                    points[2].Y = mRECT.bottom;

                                    points[3].X = mRECT.left;
                                    points[3].Y = mRECT.bottom;
